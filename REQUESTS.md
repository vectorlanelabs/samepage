...
- [ ] **CI workflow not triggering** — `.github/workflows/ci.yml` is registered (active) on `vectorlanelabs/dinnerdecider` but zero runs after 3 pushes (workflow file added via git push; GitHub quirk). Local gates are green and are the real verification; CI is belt-and-suspenders. Next: check repo/org Actions policy (possibly user-level "allow" config), or re-create the workflow via the UI and watch the next push from M1+.
