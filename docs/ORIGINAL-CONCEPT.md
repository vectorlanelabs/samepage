...
> **Preserved verbatim** from the repo's original `README.md` (the concept write-up that started this project). It is history and narrative, **not** the operative spec — several behaviors described here (spreadsheet import, yes/not-tonight/no voting, random final selection, single-dinner recording) do not match v1. The operative documents are [`CHARTER.md`](../CHARTER.md) and [`docs/PLAN-v1-mvp.md`](PLAN-v1-mvp.md). The active README describes only what is being built.
