...
| D5 | **Keep rule & roster** | The participant roster **freezes when the starter begins voting** (lobby phase first; late join is disallowed in v1). A meal qualifies iff **every required participant has an explicit `yes` vote** on it. Auto-close requires every roster member to vote on every meal; manual close treats missing votes as `no`. |
