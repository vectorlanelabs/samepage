...
- **"Times Rolled"** populated on 34 rows (counts of 1–2); 37 historical dice rolls in total.
...
### Data conventions used by the import spec

- Sheet index (1–8) → category `Tab N` (renamable in-app later).
- `Times Rolled` → preserved as `meal.legacy_rolls` (historical metadata, not displayed prominently).
- Known restaurant names → auto-tagged `takeout` on import (curated list in the import spec, not an AI guess).
